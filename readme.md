...
- [x] **Day 12: 顺序冲量 (Sequential Impulses) 与 库仑摩擦力**
  - 实现 **Sutherland-Hodgman 裁剪算法**，支持矩形 vs 矩形的多点接触（1~2个接触点）。
  - 引入库仑摩擦力模型，实现切向冲量约束：$|j_t| \le \mu \cdot j_n$。
  - 重构 `World::Step` 流程，引入迭代解算（Iterations），显著提升堆叠稳定性。
  - 优化 `Manifold` 内存布局，改用固定数组代替 `std::vector`，消除运行期内存分配。
---
...
## 🚀 Day 12 进展：顺序冲量与库仑摩擦力

### 1. 核心技术：多约束解算流水线
为了让物理世界从“弹跳球”进化为“稳固积木”，Day 12 实现了以下三大关键技术：

- **多点接触 (Multi-point Contact)**：
  在矩形碰撞中，通过裁剪算法生成两个接触点。这使得物体在落地时能产生一对平衡力矩，彻底解决了 Day 11 之前物体在单点支撑下不断晃动的问题。
- **库仑摩擦力 (Coulomb Friction)**：
  摩擦力的大小不再是随机的，而是严格受限于法向压力。通过计算切线方向（Tangent）的相对速度，施加反向冲量 $j_t$，并将其限制在 $[-\mu j_n, \mu j_n]$ 范围内。
- **顺序冲量迭代 (Sequential Impulses)**：
  采用迭代式解算（通常为 8-10 次），让碰撞冲量在复杂的堆叠结构（如方块塔）中逐层传递，达成数值上的收敛。

### 2. 综合实验分析 (物理有效性验证)

通过执行 `tests/Day12SystemTest.cpp` 并导出 `012.csv`，我们获得了以下三组关键物理数据（见下图）：

- **实验 A：摩擦力制动 (Friction Braking)**
  - **现象**：滑块以 15m/s 初始水平速度滑行。
  - **结论**：观察 `Slider VelX` 曲线，速度在摩擦力的作用下呈线性衰减，并在第 120 帧左右精准降至 0。这证明了切向冲量能模拟真实的动摩擦与静摩擦转换。
  
- **实验 B：多点着陆稳定性 (Multi-Point Landing)**
  - **现象**：长木板以 45 度角（0.785 Rad）倾斜掉落。
  - **结论**：观测 `Plank Angle` 曲线，木板在碰撞瞬间（第 120 帧）角度迅速回正。受益于双接触点提供的力矩，角度在微小震荡后**永久锁定在 0 附近**。
  
- **实验 C：堆叠稳定性 (Stacking Stability)**
  - **现象**：顶部方块掉落在底部方块上。
  - **结论**：`PosY` 曲线显示，两个方块在碰撞后高度差保持恒定，且没有任何下沉或高频抖动。这标志着迭代解算器成功模拟了“坚实”的刚体接触。

### 3. 可视化校验
![Day 12 Physical Validation](readme.assets/day12_physical_validation.png)
*上图：从上至下分别为摩擦力减速曲线、着陆角度收敛曲线、以及双体堆叠位移图。数据证明了引擎在复杂约束下的数值稳定性。*
## 💻 编译与运行
