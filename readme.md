...
### 第 4 阶段：工程验证与巅峰对决
- [x] **Day 14: 性能量化与压力测试 (Profiler & Stress Testing)**
  - **核心工具**：实现高精度 `Profiler` 类，支持多任务嵌套计时与 100 帧移动平均（Moving Average）平滑统计。
  - **RAII 集成**：通过 `ScopedTimer` 在 `World::Step` 全链路注入监控点（积分、宽相、CCD、解算器）。
  - **极限挑战**：成功运行 1000 个球体混沌碰撞测试与 2000 个方块入睡测试。
  - **效能验证**：量化 AABB 树与睡眠拦截机制，证明在静止场景下 CPU 消耗可降低 90% 以上。
---
...
### 1. 技术核心：Profiler 全链路监控
为了看清每一行代码的“含金量”，我们构建了三层性能反馈体系：
- **微秒级计时**：基于 `std::chrono::high_resolution_clock`，捕捉最细微的算法抖动。
- **状态计数器**：实时监控 `Awake Bodies`、`Active Contacts` 和 `Island Count`。
- **趋势分析**：通过移动平均值（Smoothing = 0.95），过滤掉系统调度产生的瞬时噪音，真实反映物理模拟的负载曲线。

### 2. 压力测试结果分析 (1000 Balls & 2000 Boxes)

| 监控项 | 1000 球体混沌 (V2) | 2000 盒子堆叠 (V2) | 说明 |
| :--- | :--- | :--- | :--- |
| **BroadPhase 耗时** | ~8.0 ms | ~0.02 ms (睡眠中) | AABB 树在万级对下依然稳健 |
| **Island Solver 耗时** | ~6.4 ms | ~0.01 ms (睡眠中) | 证明了睡眠拦截的绝对优势 |
| **Step Total 耗时** | **~15.2 ms** | **~0.07 ms** | 1000 物体仍能维持 60 FPS |
| **活跃碰撞对 (Max)** | 10,142 | 16,020 | 成功承受海量流形解算 |

---

### 3. 开发复盘：关于“越狱球体”与数值爆炸 (Numerical Explosion)

在 1000 个球体的压力测试中，我们观察到大量球体飞出边界。经过 `Profiler` 与数据审计，确诊为 **“重叠爆炸（Overlap Explosion）”**：

- **现象**：球体位置瞬间跳变至 $X > 100$，线速度突破几千 $m/s$。
- **根因**：随机生成（`rand()`）导致多个球在第一帧重叠。
  - **数学逻辑**：重叠产生的排斥力 $F \propto \frac{1}{\text{dist}}$。当距离趋近 0，冲量趋近**无穷大**。
  - **CCD 失效**：无穷大的速度导致位移超过了 CCD 的子步迭代上限，球体在单个步长内直接瞬移出了 5 米厚的墙。
- **应对策略**：
  1. **网格化生成 (Grid Spawning)**：从源头消灭出生重叠，确保系统初始动能受控。
  2. **速度限幅 (Velocity Clamping)**：在 `Step` 中强制设置 `maxSpeed`，为 CCD 留下可处理的物理余量。
  3. **消能偏置 (Restitution Bias)**：将弹力处理移入 `PreSolve` 并设置 `VelocityThreshold`，防止能量在复杂碰撞中无序累加。

### 4. 为什么没有生成 GIF？
由于 1000 个物体在初始重叠状态下的爆炸力极强，且高密度碰撞对离散解算器的迭代次数（8次）提出了巨大挑战，画面目前伴随剧烈的数值震荡（Jittering）。**这是物理引擎开发中的真实过程**：在解决了性能（AABB树）和逻辑（CCD）后，最后一步永远是针对极端数值精度的**调参（Tuning）**。
